...
# Besure to import out other modules
# so we can ahve access to them
import phonebook_student_main
import phonebook_student_gui



def center_window(self, w, h): # passin the tkinter frame (master) reference and the w and h
    #get user's screen width andheight
...
def ask_quit(self):
    if messagebox.askokcancel("Exits program", "Okay to exit application?"):
        # This closes app
        self.master.destroy()
        os._exit(0)
...
#===================================================================
def create_db(self):
    conn = sqlite3.connect('phonebook.db2')
    with conn:
        cur = conn.cursor()
...
def first_run(self):
    data = ('John','Doe','John Doe', '111-111-1111','jdoe@email.com', 'Python Course')
    conn = sqlite3.connect('phonebook.db2')
    with conn:
        cur = conn.cursor()
        cur,count = count_records(cur)
        if count< 1:
            cur.execute("""INSERT INTO tbl_phonebook (col_fname,col_lname,col_fullname,col_phone,col_email, col_course) VALUES (?,?,?,?,?,?)""", (data[0],data[1],data[2],data[3],data[4],data[5]))
            conn.commit()
    conn.close()
...
def addToList(self):
    ...
    var_fullname = ("{} {}".format(var_fname,var_lname)) # combine our nromalized names into a fullname
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and (len(var_email) > 0):# enforce the user to provide both
        ...
        with conn:
            ...
            if chkName == 0: # if this is 0 then there is no existance of the fullname and we can add new data
                ...
                cursor.execute("""INSERT INTO tbl_phonebook (col_fname,col_lname,col_fullname,col_phone,col_email) VALUES (?,?,?,?,?,?)""",((var_fname,var_lname,var_fullname,var_phone,var_email, var_course)))
...
def onDelete(self):
    ...
    with conn:
        cur = conn.cursor()
        # chekc count to ensure that this is not the last record in
        # the database... cannot delete last record or we will get an error
        cur.execute("""SELECT COUNT(*) FROM tbl_phonebook""")
        count = cur.fetchone()[0]
        if count > 1:
            confirm = messagebox.askokcancel("Delte Confirmation", "All information associated with ({}) \nwill be permantly deleted from the database. \n\nProceed with the deletion request?".format(var_select))
            if confirm:
                conn = sqlite3.connect('phonebook.db2')
                with conn:
                    cursor = conn.cursor()
