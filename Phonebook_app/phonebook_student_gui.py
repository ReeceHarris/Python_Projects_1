...
def load_gui(self):
    ...
    self.lstList1.bind('<<ListboxSelect>>',lambda event: phonebook_student_func.onSelect(self,event))
    ...
    self.btn_add = tk.Button(self.master,width=12,height=2,text='Add',command=lambda: phonebook_student_func.addToList(self))
    self.btn_add.grid(row=10,column=0,padx=(25,0),pady=(75,10),sticky=W)
    self.btn_delete = tk.Button(self.master,width=12,height=2,text='Delete',command=lambda: phonebook_student_func.onDelete(self))
    self.btn_delete.grid(row=10,column=1,padx=(25,0),pady=(75,10),sticky=W)
    self.btn_close = tk.Button(self.master,width=12,height=2,text='Close',command=lambda: phonebook_student_func.ask_quit(self))
    self.btn_close.grid(row=10,column=2,columnspan=1,padx=(15,0),pady=(75,10),sticky=E)
    
    phonebook_student_func.create_db(self)
    phonebook_student_func.onRefresh(self)
